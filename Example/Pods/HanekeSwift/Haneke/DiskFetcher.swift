...
open class DiskFetcher<T : DataConvertible> : Fetcher<T> {
    ...
    open override func fetch(failure fail: ((NSError?) -> ()), success succeed: @escaping (T.Result) -> ()) {
    ...
    fileprivate func privateFetch(failure fail: ((NSError?) -> ()), success succeed: @escaping (T.Result) -> ()) {
        if self.cancelled {
            return
        }
